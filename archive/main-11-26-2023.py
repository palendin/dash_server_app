...
def update_project_id(experiment_id):
    return f"{experiment_id} =project ID"
    ...
    if xaxis_column and yaxis_columns is not None:
        ...
        if len(experiment_id) >= 2:
            ...
            for i,yaxis_column in enumerate(yaxis_columns):
                ...
                fig.add_trace(go.Bar(x=data_filtered[xaxis_column],y=data_filtered[yaxis_column],name=yaxis_column,yaxis= yaxis_num[i])) #marker_color='#d99b16',hoverinfo='none'))
