...
from pg_query import query_biopsyresult
...
app.layout = html.Div([
        ...
        dbc.Row([dbc.Col(html.H1('Vitrolab Dashboard', style={'textAlign': 'center'}))]), # title
        ...
        dbc.Row([html.Button('Submit', id='submit button', n_clicks=0)]),
        html.Div(id='button output', children=''),  
        dcc.Store(id='intermediate-value'),
        html.Br(),
        
        # components for button for template
        dbc.Row([dbc.Col(dcc.RadioItems(id='template', options = [{'label': k, 'value': k} for k in templates], value = None, inline=True))]), # grid style
        html.Br(),

        # components for graph, graph options, and download data from plot
        ...
        html.Br(),
         ...
         # components for button for template for graph1
        # dbc.Row([dbc.Col(dcc.RadioItems(id='template1', options = [{'label': k, 'value': k} for k in templates], value = None, inline=True))]), # grid style
        # html.Br(),

        # components for 2nd graph. graph options, and download data from plot
        # dbc.Row([dbc.Col(dcc.Graph(id="graph1", figure=blankfigure())),]),
        # dbc.Row([dbc.Col(dcc.RadioItems(id = 'plot_type1', options = ['line','bar'], value = 'line', inline=True, style={'font-size':20}))]),
        # dbc.Row([dbc.Col(html.Button("Download Plot Data", id="download_plot_data_button1")),
        #          dbc.Col(dcc.Download(id="download-plot-data-csv1"))
        #         ]),

        # components for upload file 
        dbc.Row([dbc.Col(html.Label('Upload file'), style={'textAlign': 'left'})]),
